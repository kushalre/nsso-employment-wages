...
setwd("/media/kushal/001A3CCE1A3CC308/kushal/research/nsso-employment/revised-nsso-apr11/NSS_38")

#Import data of NCO-1968
#codes <- read.csv("/home/kushal/research/nsso-employment/india-nsso/codes-v2.csv")
...
# Rural Data 
##########################
rural_data <- read.csv("rural-38.csv")

levels(rural_data$nco)
rural_datax<-rural_data[(rural_data$nco==c("X80","X09")),]
...
rural_df <- rural_data_final[,c(1,54)]
...
# for(i in 1:nrow(mydata_rural)){
#   p<- mydata_rural$Var1[i]
#   mydata_rural$Freq[i]<-sum(rural_data_final$pooledweight[rural_data_final$occupationcode==p],na.rm=TRUE)
# }
...
saveRDS(mydata2_rural, file = "/media/kushal/001A3CCE1A3CC308/kushal/research/nsso-employment/revised-nsso-apr11/rural38.rds")
...
#Urban Data
#########################
urban_data <- read.csv("urban-38.csv")
...
urban_df <- urban_data_final[,c(1,54)]
...
# for(i in 1:nrow(mydata_urban)){
#   p<- mydata_urban$Var1[i]
#   mydata_urban$Freq[i]<-sum(urban_data_final$pooledweight[urban_data_final$occupationcode==p],na.rm=TRUE)
# }
...
saveRDS(mydata2_urban, file = "/media/kushal/001A3CCE1A3CC308/kushal/research/nsso-employment/revised-nsso-apr11/urban38.rds")
